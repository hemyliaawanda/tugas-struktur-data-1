...
void tampil(struct queue *q)
{
	...
	x=q->front;
	printf("-------------------------------------------\n");
	printf("Data antrian mobil yang telah parkir : \n");
	printf("-------------------------------------------\n");
	if(q->count==0)
		printf("\n Maaf Tidak ada mobil yang sedang parkir\n");
	else {
		for(i=1;i<=q->count;i++){
	...
	printf("\n\n =>> Tekan tombol lain untuk kembali ke pilihan <<=");
	getch();
	return;
}
...
int main()
{
	...
	printf("***************************\n");
	printf("1. Antrian parkiran mobil\n");
	...
	if(jawab==1){
		printf("Masukkan plat nomor mobil yang masuk (tanpa huruf) : "); scanf("%d",&plt);
		masuk(plt,&q);
		tampil(&q);
	}
	else if(jawab==2){
		printf("Masukkan plat nomor mobil yang keluar (tanpa huruf):"); scanf("%d",&plt);
		keluar(plt,&q,&s);
		tampil(&q);
	}
	else if(jawab==3){
